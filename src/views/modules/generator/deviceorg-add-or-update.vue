    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.deviceid) {
            this.$http({
              url: this.$http.adornUrl(`/generator/deviceorg/info/${this.dataForm.deviceid}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.orgname = data.deviceOrg.orgname
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/generator/deviceorg/${!this.dataForm.deviceid ? 'save' : 'update'}`),
              method: 'post',
