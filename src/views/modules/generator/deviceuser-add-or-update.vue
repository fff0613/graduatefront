    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.deviceid) {
            this.$http({
              url: this.$http.adornUrl(`/generator/deviceuser/info/${this.dataForm.deviceid}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.userid = data.deviceUser.userid
                this.dataForm.role = data.deviceUser.role
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/generator/deviceuser/${!this.dataForm.deviceid ? 'save' : 'update'}`),
              method: 'post',
