  ...
  <div>
    <el-row>
      ...
      <el-button
        size="small"
        type="primary"
        @click="$router.push({ name: 'device-scrap' })"
        >报废</el-button
      >
      <!-- </el-col> -->
    </el-row>
    ...
    <el-table :data="tabledata.slice((pageIndex - 1) * pageSize, pageIndex * pageSize)" style="width: 100%;margin-top:20px" border height="550">
       <el-table-column fixed prop="recordid" label="报废单号"
...
export default {
  ...
  methods: {
     getDataList() {
      this.tabledata = [];
      this.$http({
        url: this.$http.adornUrl("/liferecord/scrap"),
        method: "get",
    ...
    sizeChangeHandle(val) {
     ...
     search(){
      ...
      this.$http({
              url: this.$http.adornUrl("/liferecord/scrap/search"),
              method: "post",
