<template>
  <div>
    ...
    <el-table :data="tabledata.slice((pageIndex - 1) * pageSize, pageIndex * pageSize)" style="width: 100%" height="550">
      <el-table-column fixed prop="returnid" label="退库单号"
        ><template slot-scope="scope"
          ><el-button
            v-model="scope.row.returnid"
            type="text"
            @click="
              $router.push({
                name: 'life-ifo',
                query: {
                  recordid: scope.row.returnid,
                  state: scope.row.state,
                },
              })
            "
            >{{ scope.row.returnid }}</el-button
          ></template
        >
      </el-table-column>
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-button
            ...
            @click="
              $router.push({
                name: 'life-ifo',
                query: {  recordid: scope.row.returnid,
                  state: scope.row.state, },
              })
            "
            >详情</el-button
...
export default {
  ...
  methods: {
    getDataList() {
      this.tabledata = [];
      this.$http({
        url: this.$http.adornUrl("/liferecord/stockreturn"),
        method: "get",
