<template>
  <div>
    <el-row>
      <!-- <el-col :sapn="12"> -->
      <el-select v-model="option.stock" placeholder="转移后库房" class="select" @change="selectchanged">
        <el-option
    ...
    <el-table
      ...
      height="550"
    >
      <el-table-column prop="transferid" label="转移单号" 
        ><template slot-scope="scope"
          ><el-button
            v-model="scope.row.transferid"
            type="text"
            @click="
              $router.push({
                name: 'life-ifo',
                query: { recordid: scope.row.transferid },
              })
            "
            >{{ scope.row.transferid }}</el-button
          ></template
        >
      </el-table-column>
      <el-table-column prop="newstock" label="所属库房"
        ><template slot-scope="scope"
          ><el-button
            v-model="scope.row.newstock"
            type="text"
            @click="$router.push({ name: 'stock-manage' })"
            >{{ scope.row.newstock }}</el-button
          ></template
        >
      </el-table-column>
      <el-table-column prop="oldstock" label="原库房" 
        ><template slot-scope="scope"
          ><el-button
            v-model="scope.row.oldstock"
            type="text"
            @click="$router.push({ name: 'stock-manage' })"
            >{{ scope.row.oldstock }}</el-button
          ></template
        >
      </el-table-column>
      <el-table-column
...
export default {
  ...
  methods: {
    getDataList() {
      this.tabledata = [];
      this.$http({
        url: this.$http.adornUrl("/liferecord/transfer"),
        method: "get",
        ...
        if (data && data.code === 0) {
          ...
          for (let item of this.tabledataRe) {
            let t = {
              transferid: item.recordid,
              newstock: item.stockname,
              oldstock: item.oldstockname,
    ...
    search(){
      ...
      this.$http({
              url: this.$http.adornUrl("/liferecord/transfer/search"),
              method: "post",
