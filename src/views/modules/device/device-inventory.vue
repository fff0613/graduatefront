<template>
  <div>
    <el-row>
      ...
      <!-- </el-col> -->
      <!-- <el-col > -->
      <el-input v-model="input" placeholder="执行人" style="width: 200px">
        <el-button
          slot="append"
          icon="el-icon-search"
          type="primary"
    ...
    <el-table :data="tabledata.slice((pageIndex - 1) * pageSize, pageIndex * pageSize)" style="width: 100%;margin-top:20px" border height="550">
      <el-table-column fixed prop="recordid" label="盘点单号" 
      ...
      <el-table-column prop="stock" label="所属库房"
        ><template slot-scope="scope"
          ><el-button
        ...
        <template slot-scope="scope">
          <el-button
            ...
            @click="
              $router.push({
                name: 'life-ifo',
                query: { recordid: scope.row.recordid,state: scope.row.state,},
              })
            "
            >详情</el-button
          >
...
export default {
  ...
  methods: {
     getDataList() {
      this.tabledata = [];
      this.$http({
        url: this.$http.adornUrl("/liferecord/inventory"),
        method: "get",
    ...
    sizeChangeHandle(val) {
     ...
     search(){
      ...
      this.$http({
              url: this.$http.adornUrl("/liferecord/inventory/search"),
              method: "post",
              ...
              if (data && data.code === 0) {
                ...
                for (let item of this.tabledataRe) {
                  let t = {
                    ...
                    organization: item.orgname,
                    mount: item.amount,
                    reporter: item.reporteranme,
                    happentime: item.happentime,
                    state: item.state,
                    executor: item.executoranme,
                  };
                  this.tabledata.push(t);
                }
                this.totalPage = this.tabledata.length;
