<template>
  <div>
    <el-row>
      <!-- <el-col > -->
      <el-select v-model="option.stock" class="select">
        <el-option
    ...
    <el-table :data="tabledata.slice((pageIndex - 1) * pageSize, pageIndex * pageSize)" style="width: 100%" height="550">
      <el-table-column fixed prop="recordid" label="归还单号" 
     ...
     <el-table-column prop="stock" label="目标库房"
        ><template slot-scope="scope"
          ><el-button
...
export default {
  ...
  methods: {
     getDataList() {
      this.tabledata = [];
      this.$http({
        url: this.$http.adornUrl("/liferecord/giveback"),
        method: "get",
